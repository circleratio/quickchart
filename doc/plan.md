...
### 並べ替え時に再配置が追従しない不具合(構造化テンプレート) — 対応済み

パターン定義(`PatternDefinition`、spec §6.2)を整理した際に見つかったもの。字下げ/字上げ・並べ替えで「既存図形を位置だけ移動する」方式を使っていたパターンのうち、次の4つで見た目が崩れていた(`sync.test.ts` の「restructure matches a fresh layout of the new outline」で再現)。

- **見出し付き箇条書き / 箇条書きマトリクス**: 行の区切り線・グリッド線がノードに結びつかない図形のため移動せず、高さの異なる行を入れ替えると古い位置に残った。
- **４象限マトリクス**: バッジの塗り色が並び順で決まるため、象限を並べ替えても色が追従しなかった。spec §6.2.1 の「並べ替えも全体を再生成する」とも食い違っていた。
- **ベン図**: 円と集合名ラベルが同じノードに結びついているため、位置だけ移動すると円がラベルの位置へ動いていた。

**対応**: 位置だけ移動する方式はツリー図/ロジックツリーに限定し、それ以外のパターンは構造を変える編集のたびに全体を再生成するようにした。パターンごとに方式を選ぶ `PatternDefinition.restructure` は、誤った選択が今回の原因だったため廃止した(`tree` の有無で決まる)。並べ替え時に、手動で変更した図形のスタイルが失われるのは他の再生成パターンと同じ扱い。
