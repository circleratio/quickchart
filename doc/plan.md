...
### 並べ替え時に再配置が追従しない不具合の候補(構造化テンプレート)

パターン定義(`PatternDefinition`、spec §6.2)を整理した際に見つかったもの。`restructure: "relayout"` は「既存図形を位置だけ移動する」方式であり、並び順に依存する図形や、どのノードにも結びつかない図形(`nodeIds: []`)を持たないことが前提になる。以下のパターンはこの前提に反している可能性がある。いずれも未確認・未修正。

- **見出し付き箇条書き / 箇条書きマトリクス**: 行の区切り線(箇条書きマトリクスはグリッド線も)がノードに結びつかない図形のため、並べ替えても移動しない。高さの異なる行を入れ替えると、区切り線が古い位置に残る可能性がある。
- **４象限マトリクス**: バッジの塗り色が並び順で決まる(`matrix.ts` の `BADGE_FILLS`)。象限を並べ替えても色が追従しない可能性がある。

**対応案**: 該当パターンの定義を `restructure: "regenerate"` に変更する。この場合、並べ替え時に手動で変更した図形のスタイルは失われるが、`"regenerate"` にしている他のパターンと同じ扱いになる。挙動が変わるため、リファクタリングとは別のコミットで行う。
