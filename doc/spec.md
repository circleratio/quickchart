...
「ピラミッドストラクチャー」(UI上の表示名は「ツリー図」)は、親子関係を示す接続線(エルボー型:親の下辺中央から下に伸び、子たちの中心を結ぶ横線("バス")を経て、各子の上辺中央へ下りる)を薄いグレー(テーマに依存しない固定色、`style.ts` の `treeConnectorStyle`)で描画する。バスの高さは親の下辺と子の上辺のちょうど中間(縦2本が等しい長さになる位置)に置く固定オフセットではなく都度計算する。子が1つだけの場合はバス部分(横線)を省略し、縦の2本だけになる。接続線は `templateNodeIds: []` の非追跡図形として扱い(箇条書きマトリクスのグリッド線と同じ扱い)、`StructuredBlock.params._treeConnectorShapeIds` で管理する。ピラミッドは他パターンと異なり階層テキストの追加/削除を増分配置(`buildNodeShape`)で行うため、接続線は各ノードの**実際の現在位置**(`treeLayout.ts` の理想化されたカーソル配置ではなく、手動での位置調整結果も含む)から都度再計算し(`sync.ts` の `regenerateTreeConnectors`)、追加・削除・字下げ/字上げ・並べ替え・一括インポートのいずれの操作後にも呼び出す。ロジックツリーなど他パターンには適用しない。ブロック間の距離(増分配置のオフセット `newNodeOffset`、および `layoutTree` ベースの一括生成/再配置で使う `pyramid.ts` の `GAP`)は、ロジックツリーの対応する値の1.5倍にしてある。
